...
public partial class TelemetryMainWindow
{
    ...
    {
        base.OnSourceInitialized(e);Title="CoreWatch";foreach(var text in Descendants<TextBlock>(this)){if(text.Text=="SYSTEM CHECKER")text.Text="COREWATCH";else if(text.Text=="SC")text.Text="CW";}var iconPath=Path.Combine(AppContext.BaseDirectory,"Assets","CoreWatch.ico");if(File.Exists(iconPath)){_coreWatchTrayIcon=new Icon(iconPath);_tray.Icon=_coreWatchTrayIcon;}_tray.Text="CoreWatch · Local Telemetry";Closed+=(_,_)=>_coreWatchTrayIcon?.Dispose();ImproveThermalLayout();WrapReportInScroller();EnhanceBenchmarkReference();EnhanceReportContents();InitializeProcessPage();ApplyTaskManagerDesign();
    }
    private void ImproveThermalLayout(){var temperature=Descendants<TextBlock>(this).FirstOrDefault(t=>BindingOperations.GetBinding(t,TextBlock.TextProperty)?.Path.Path=="TemperatureValue");if(temperature?.Parent is StackPanel stack&&stack.Parent is Grid grid&&grid.ColumnDefinitions.Count>=3){grid.ColumnDefinitions[0].Width=new GridLength(245);temperature.FontSize=15;var existing=grid.Children.OfType<Button>().FirstOrDefault(b=>Equals(b.Content,"센서 권한 재시작"));if(existing is not null){grid.Children.Remove(existing);var actions=new StackPanel{Orientation=Orientation.Horizontal,Margin=new Thickness(12,0,0,0)};Grid.SetColumn(actions,2);existing.Margin=new Thickness(0,0,8,0);actions.Children.Add(existing);var pawn=Action("PawnIO 센서 드라이버",PawnIo_Click,"#E4E7EB","#303640");pawn.ToolTip="Windows 메모리 무결성과 호환되는 공식 저수준 센서 드라이버 설치 페이지";actions.Children.Add(pawn);grid.Children.Add(actions);}}}
