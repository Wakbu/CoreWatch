...
- 스크롤바는 14px 트랙, 최소 44px thumb, 실제 Minimum/Maximum/Value/ViewportSize 바인딩을 사용합니다.\n- 세로 스크롤바 Height는 Auto로 전체 페이지 높이를 사용하며 가로 스크롤바만 14px 높이를 사용합니다.\n- 모든 DataGrid 셀과 헤더에 수직 중앙 정렬 스타일을 강제 적용합니다.\n- 프로세스 메모리 StringFormat은 `{0:N0} MB`로 교정했습니다.
...
- 설치 프로그램 관리자 매니페스트 포함 확인\n- 리포트 세로 스크롤바 14×760px, ViewSize 58.6%, 75% 위치 이동 확인\n- 프로세스 셀 중심 오차 0px, 하드웨어 셀 최대 중심 오차 0.5px\n- 프로세스 메모리 값에 잘못된 `N` 문자 없음 확인
